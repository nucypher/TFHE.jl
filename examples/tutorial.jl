...
function prepare()
    ...
    ciphertext1 = [encrypt(rng, secret_key, bits1[i]) for i in 1:16]

    # generate encrypt the 16 bits of 42
    plaintext2 = UInt16(42)
    bits2 = int_to_bits(plaintext2)
    ciphertext2 = [encrypt(rng, secret_key, bits2[i]) for i in 1:16]
...
function encrypted_minimum(ck::CloudKey, a::Array{LweSample, 1}, b::Array{LweSample, 1})
    ...
    for i in 1:nb_bits
        tmps1 = encrypted_compare_bit(ck, a[i], b[i], tmps1)
    end

    # tmps1 is the result of the comparaison: 0 if a is larger, 1 if b is larger
    # select the max and copy it to the result
    [gate_mux(ck, tmps1, b[i], a[i]) for i in 1:nb_bits]
end
